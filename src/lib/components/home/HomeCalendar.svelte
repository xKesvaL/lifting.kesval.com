...
<div class="btn-ghost btn my-2 h-auto w-full bg-base-200 px-4 py-4 normal-case shadow-lg">
	...
	<div class="grid w-full grid-cols-7 justify-items-center gap-1">
		{#each { length: 7 } as __, i}
			<div class="mb-2 text-center">{$_('calendar.day.' + i)}</div>
		{/each}
		{#each { length: daysInMonth } as __, ind}
			<div class="aspect-square max-w-[2rem] rounded-full bg-base-100">
				{#if ind % 5 === 0}
					<div
